...
sources = {"2016":"default2016","2017":"default","2018":"default2018"}
	
for src, path in sources.iteritems():
	
	for rap in ["BB","BE"]:
	
		graphInner    = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("Inner",rap))
		graphOuter    = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("Outer",rap))
		graphGlobal   = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("Global",rap))
		graphTPFMS    = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("TPFMS",rap))
		graphPicky    = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("Picky",rap))
		graphDYT      = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("DYT",rap))
		graphTunePNew = graph(path,"MassResolutionVsMass_%s_%s.pkl"%("TunePNew",rap))
		...
		canv.Print("massResolutionCompare_%s_%s.pdf"%(src,rap))
