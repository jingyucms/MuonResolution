...
# ~ mrange = [120, 200, 300, 400, 600, 800, 1000, 1300, 1600, 2000, 2500, 3100]
mrange = [30, 40, 50, 75, 100, 150, 200, 350, 500, 2000]
...
sampleLists  = {
	...
	"2017MassBinned":["dileptonAna_resolution_dy50to120_2017_MUOTrackFix.root","dileptonAna_resolution_dy120to200_2017_MUOTrackFix.root","dileptonAna_resolution_dy200to400_2017_MUOTrackFix.root","dileptonAna_resolution_dy400to800_2017_MUOTrackFix.root","dileptonAna_resolution_dy800to1400_2017_MUOTrackFix.root","dileptonAna_resolution_dy1400to2300_2017_MUOTrackFix.root","dileptonAna_resolution_dy2300to3500_2017_MUOTrackFix.root","dileptonAna_resolution_dy3500to4500_2017_MUOTrackFix.root","dileptonAna_resolution_dy4500to6000_2017_MUOTrackFix.root","dileptonAna_resolution_dy6000toInf_2017_MUOTrackFix.root"],
...
def loadHistos(inputfiles,region,rebin,trackType,weights):
	...
	for k,mc in enumerate(_file):
		#~ print _file[k].ls()
		if ("B" in region):
			tmp   = _file[k].Get("%s/LeptonInvPtResVPtGen_2d_BO" %(histoname)).Clone()
		elif ("O" in region):
			tmp   = _file[k].Get("%s/LeptonInvPtResVPtGen_2d_O" %(histoname)).Clone()
		elif ("E" in region):
			tmp   = _file[k].Get("%s/LeptonInvPtResVPtGen_2d_E2" %(histoname)).Clone()
		tmp.Sumw2()
...
#            funct.SetParLimits(1, gaus.GetParameter(1)*0.5, gaus.GetParameter(1)*1.5)
		...
		h.SetTitle("p_{T} resolution for %d < p_{T} <%d" %(mrange[i],mrange[i+1]))
		h.GetXaxis().SetTitle("p_{T}^{RECO} / p_{T}^{GEN} - 1")
		...
		saveas = "/PtRes_Pt%d_%d_%s_%s" %(mrange[i],mrange[i+1],rap,fit)
...
def makeMassRes(inputfile,output,funct,trackType,weights):
	...
	resB.SetFillColor(0)
	resB.SetTitle("Muon p_{T} resolution vs p_{T}")
	resB.GetYaxis().SetTitle("Muon p_{T} Resolution")
#    resBB.GetYaxis().SetTitleOffset(1.5)
	resB.GetXaxis().SetTitle("p_{T}^{#mu} [GeV]")
	...
	resO.SetFillColor(0)
	resO.SetTitle("Muon p_{T} resolution vs p_{T}")
	resO.GetYaxis().SetTitle("Muon p_{T} Resolution")
 ...
 #   resBE.GetXaxis().SetTitle("m(#mu^{+}#mu^{-}) [GeV]")
	...
	resE.SetFillColor(0)
	resE.SetTitle("Muon p_{T} resolution vs p_{T}")
	resE.GetYaxis().SetTitle("Muon p_{T} Resolution")
