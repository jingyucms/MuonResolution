...
def compareMassRes(trackType):
	...
	cat = ["B","O","E"]
	for c in cat:
		fileDefaultBB = open("defaultPSplit/PResolutionVsP_%s_%s.pkl"%(trackType,c))
		fileORBB = open("Rebin2PSplit/PResolutionVsP_%s_%s.pkl"%(trackType,c))
		fileNoBB = open("Rebin8PSplit/PResolutionVsP_%s_%s.pkl"%(trackType,c))
		...
		ratioORBB.SetMarkerColor(kRed)
		ratioNoBB.SetMarkerColor(kBlue)

		ratioPad.DrawFrame(0,0.8,3100,1.2,";;ratio")

		ratioORBB.Draw("samepe")
		ratioNoBB.Draw("samepe")

		canv.Print("pResolutionVsPBinning_%s_%s.pdf"%(trackType,c))
